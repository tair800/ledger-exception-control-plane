...
| **Human-labelled hold-out slice** | **PENDING OWNER LABEL REVIEW — packet issued** | 25 records, frozen at `hold_out_sha256 cfa62715…`, hold-out version 1. The owner-facing workbook is at `artifacts/human-label-packet.xlsx` with a `LABEL_GUIDE` sheet; `HUMAN_LABEL` is blank in all 25 rows and no program can fill it. The importer accepts `.xlsx`, `.csv` and `.jsonl` and refuses an unknown id, a duplicate, a blank, an invalid code, a changed fingerprint or a stale version. Every record still says `label_source: derived`, and a test asserts it. **Measured limit (ADR-067): the derived label is a pure function of the classification across all 250 records, so the slice carries four independent judgements, not 25.** **OPEN-15.** |
