...
**The model measurement is a negative result and is reported as one.** 27.9% accuracy over the
247 records it answered, against an 85.6% constant-answer baseline; 97.2% on the 36 records where a
proposal changes what happens; 177 of the 211 escalate-labelled records it answered came back with
a concrete treatment instead. **Three fail-closed controls caught different parts of that, and the
approval gate is only one of them** — the citation check refused one hallucinated evidence id, the
account policy refuses all 177 (`NO_ACCOUNT_MAPPED`, which is the same fact that makes the correct
label `escalate`), and the gate stands in front of the single wrong answer that would really have
priced (ADR-070).
...
|---|---|---|
| **Live model quality and latency** | **MEASURED 2026-09-10 (ADR-070)** | One bounded run, 250 records, **251 live calls** against a declared ceiling of 750, through an OpenAI-compatible OmniRoute route (alias `auto/best-free`; every response named `gpt-5.5`, and the physical upstream is not verifiable from here so it is not claimed). **Usable through the shipped path 247/250 = 98.8%** — 2 truncated tool arguments and 1 hallucinated evidence id, all refused rather than coerced. **Accuracy 27.9% over the 247 answered against an 85.6% constant-answer baseline — a −57.7% lift — and 97.2% on the 36 priceable records (35 of 36).** Abstention 13.8%, none of it on a priceable case. Latency min 2.20s / p50 4.80s / p95 8.15s / max 17.97s. Tokens 799,492 in, 46,703 out. **The finding: the model is good at the judgement and bad at declining to make one** — 177 of the 211 escalate-labelled records it answered got a concrete treatment. **Three controls caught different parts of that and only the last is the gate:** the citation check refused the hallucination; the account policy refuses all 177, because `unclassified` and `fee_split` have no account mapped and `compute_adjustment` returns `NO_ACCOUNT_MAPPED` before an amount exists; the approval gate stands in front of the one wrong answer that would really have priced. `live-eval --from-cassette` recomputes every figure offline with no credential. |
...
there is no real ledger adapter, so the strong-configuration column proves the dispatcher behaves
correctly *given* an enforcing ledger rather than that any particular ledger enforces anything
(**OPEN-11**).
...
| **6.4 Live model evaluation** | **DONE — and a negative result** | One bounded run over all 250 golden records: 251 live calls against a declared 750 ceiling, **98.8% usable through the shipped path**, **97.2% accurate on the 36 priceable records**, **27.9% over the 247 answered against an 85.6% constant-answer baseline**, abstention 13.8% with none on a priceable case, p95 8.15s, 846,195 tokens. The model is good at the judgement and bad at declining to make one: 177 of the 211 escalate-labelled records it answered got a concrete treatment — contained by three separate fail-closed controls, of which the approval gate is the one covering the single answer that would really have priced. An OpenAI-compatible route accepted `response_format` and did not enforce it — found by probing *before* the run, so a tool-calling envelope was added rather than a gateway limitation being published as a model score. The guard keeping `src/` free of HTTP clients was **not** weakened: the transport lives under `tests/`. ADR-070 |
...
**A labelled set of 250 exceptions, and a scorer that a constant answer cannot flatter.** §20 asks
for accuracy, an abstention rate and a confusion matrix. All three are here, and so are three
figures §20 does not name — because on this set the three it does name would mislead.

### The set's shape is a fact about the system, not a sampling choice

Only four classes are reachable, and the account policy configures an account for **two** of them.
For `fee_split` and `unclassified` there is nothing to post, so the correct action is to refer the
case to a human — which makes 214 of the 250 labels `ESCALATE`. The proportion is stable at every
scale tried (84.6% at 200 scenario instances, 85.8% at 600, 85.6% at 1200, 85.5% at 2400), so it is
structural and no amount of corpus will change it.

**A model that answers `ESCALATE` to everything is therefore 85.6% accurate while deciding
nothing.** Everything below follows from refusing to publish that number on its own.

### What the scorer reports beyond §20

- The **constant-answer baseline**, computed from the labels, and the lift over it. Zero for the
  constant; negative for anything worse than one.
- **Accuracy on the priceable records** — 36 of 250, the only ones where the answer changes what
  happens. The constant scores 0.0% there.
- **Abstention split by whether escalating was correct**: abstaining on a fee split is right,
  abstaining on a chargeback reversal is a refusal to do the job, and one averaged rate hides both.

A test drives the constant answerer against the committed set and asserts all three exposures,
including the exact wording of the headline. Four adversarial answerers are tested in total — a
constant, an inverted one, one that skips half the set, one that answers records not in it.

### The labels are derived from what the system concluded

A golden record carries no `scenario_id` and no `intended_classification`, and two tests enforce it —
one on the record shape, one parsing the generator's own imports. The corpus *intends*
`partial_capture` for two scenarios the classifier assigns something else entirely, so a label read
off the construction metadata would grade a model against a fact it was never shown.

Five of the generator's six steps are calls into shipped modules. Only the label clause is the
harness's own judgement, and it states a reason per clause that a reviewer can disagree with.

### The number it refuses to publish

The committed cassettes are **synthesised** — their bodies say "Not produced by a model". `score()`
takes a required `origin` with no default, and a synthesised run's headline contains *"THIS IS NOT A
MODEL MEASUREMENT"* and says "agreement" rather than "accuracy". The wording is load-bearing:
whoever copies that line into a README will not copy a caveat from a docstring.

### What needs the owner

§20 wants a **human-labelled** hold-out slice. The slice is selected deterministically (25 records,
fixed stride, same on every run), the mechanism to record a human label exists, and every committed
record says `derived` because nobody has confirmed one. A test pins that gap and is written to fail
on the day it closes. Recorded as **OPEN-15**.
