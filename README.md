...
**Money.** Every monetary column is `NUMERIC(20, 4)` mapped to Python `Decimal`. Binary floating
point is absent from the schema and a test asserts it across all metadata, not just the known money
columns. Scale 4 so no ISO 4217 minor unit rounds at the storage boundary. Every amount is paired
with an explicit currency column, and a check constraint makes the two both-present-or-both-absent.
